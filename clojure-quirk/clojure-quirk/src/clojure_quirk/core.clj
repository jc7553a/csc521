...
(def parser
	(insta/parser 
		(slurp  "resources/Quirk-EBNF.txt") 
...
(defn FunctionCall [subtree scope]
	...
	(def funcbody (second (get scope funcName)))
	 (first (CallByLabel (first funcbody)funcbody funcscope)))		
	)
...
(defn Name [subtree scope]
		...
		(= 2 (count (second subtree)))
			(cond (= nil (get scope (second (second (second subtree)))))
				(str (second(second subtree)))
				:else
					(get scope (second (second (second subtree)))))

		(= 3 (count (second subtree)))
			(cond (= nil (get scope (second (third (second subtree)))))
